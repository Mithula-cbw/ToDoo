...
const TaskCard:React.FC<taskCardProps> = ({ taskId, title, date ,editTask, priority, taskStatus ,onDelete}) => {
    ...
    const priorityColor = {
      ...
      const [status, setStatus] = useState<string>("pending"); // default status can be 'pending' or any value based on initial status
                ...
                <AccordionTrigger>
                    <div className="w-full h-full flex flex-row gap-5 justify-between items-center">
                        <div className="w-full h-full flex flex-row gap-5 justify-start items-center">
                            <button className={`w-4 h-4 md:w-6 md:h-6 rounded-full border-2 border-primary-yellow ${status==='completed' ? "bg-primary-yellow" : "bg-transparent"}`}
                            onClick={handleStatusChange}>
                            </button>
                            <div className="flex flex-col md:gap-1">
                                <div className={`font-medium font-poppins text-sm md:text-lg w-full md:h-6 bg-gray-half rounded-md ${status==='pending'? "" : "text-decoration: line-through text-gray-500" }`}>
